...
- [Package.swift](../Package.swift) currently identifies `LocalDictation`, targets macOS 15+, and pins WhisperKit `0.15.0`, FluidAudio `0.14.3`, GRDB `7.10.0`, TOMLKit `0.6.0`, and Hummingbird `2.26.0`. This is source state, not runtime proof.
...
- Desktop audio remains local. Telemetry, desktop cloud speech-to-text, Overseed services, and automatic update checks are removed rather than hidden behind settings.
- An optional iPhone service may accept Access-protected audio through Cloudflare, prefer the Mac's local engine, and visibly fall back to Workers AI. It is disabled by default and is not part of desktop dictation.
- An optional web app served by the same gateway at `/app/` may record from the iPhone and present one searchable history that merges desktop and iPhone entries with edit, pin, delete, copy, and export. On capable devices it streams in-memory 16 kHz PCM through a short-lived signed WebSocket ticket for live preview and overlapped transfer while retaining the ordinary recording as automatic file-upload fallback; the existing batch engine remains authoritative after Stop. Its shell is public and credential-free; its API calls use a dedicated Access service token held only on the phone. Merging desktop history into it requires the separately opt-in, off-by-default Unified iPhone History setting; while that setting is off, no remote transcript is persisted and the history routes refuse. The Mac remains the authoritative history store, Cloudflare transports history without persisting it, and the phone keeps only a cache. The normative contract is [unified-history.md](unified-history.md).
