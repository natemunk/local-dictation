...
| IPH-005 | Three distinct service tokens isolate the Shortcut, the iPhone web app, and the Worker-to-origin path. The gateway application is path-scoped to `/v1`, `/app/` loads anonymously with a Content-Security-Policy header, and every cross-use is rejected. Public `/stream` requires a same-origin 30-second signed ticket minted through protected `/v1`; no Access token enters the socket URL. | The ten anonymous/authenticated/ticket-refusal probes in `scripts/configure-iphone-endpoint.sh`, ticket tamper/expiry tests, plus Cloudflare application and policy readback. | Probe and ticket logic are automated; pending deployment, readback, and a per-token revocation exercise. |
