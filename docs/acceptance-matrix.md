...
Snapshot: 2026-09-02. “Implemented” refers only to source in this checkout. “Automated-verified” refers to the 182-test SwiftPM run across 28 suites on this host. “Fixture-verified” refers only to synthetic scorer fixtures. “Pending” and “blocked” rows are not product claims.
...
| HIS-004 | Local analytics, destination metadata, transcript history, and full-data deletion have explicit independent controls; opt-out never blocks dictation. | Preference, destination-redaction, actor-deletion, retention, and concurrent read-only SQLite tests. | Automated-verified; Settings runtime exercise pending. |
...
| TOOL-002 | Direct SwiftPM invocation through the Xcode toolchain works when supplied the SDK plus Developer Framework search paths, even though `xcrun` remains broken. | Current run built the app and passed 182 tests in 28 suites. | Automated source contracts are usable on this host; the documented setup script contains the workaround. Repairing Xcode remains advisable for normal tooling. |
