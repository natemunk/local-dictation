...
Snapshot: 2026-09-07. “Implemented” refers only to source in this checkout. “Automated-verified” refers to the 219-test SwiftPM run across 34 suites and 289-test Worker/PWA run across 15 files on this host. “Fixture-verified” refers only to synthetic scorer fixtures. “Pending” and “blocked” rows are not product claims.
...
| UX-006 | Escape cancels active/in-flight dictation and discards temporary audio; dismissing rewrite instructions preserves the originating message in raw Preview. | Unit plus temporary-file integration test. | Generation detachment and immediate restart are automated-verified; finish capture is session-owned and rechecks generation after AX suspension. App/file lifecycle still needs manual integration. |
...
| INS-001 | Clipboard+Command+V leaves transcript text current, uses no unrestricted PID-only fallback or fixed paste delay, and never overwrites a concurrent copy. | Pasteboard/destination policy tests. | Automated-verified for exact/focus-token/focused-window tiers, bounded transient-AX retry, PID-checked system-wide focus fallback, reviewed app bundles, persistent ordinary/private clipboard data, focus loss, and concurrent clipboard changes; cross-app matrix pending. |
...
| HIS-001 | Raw transcript is saved before cleanup/insertion in GRDB/SQLite FTS5 when healthy; a store outage uses clipboard recovery and does not disable delivery. | Transaction-order integration, injected outage, and crash injection. | Source-observed with automated raw-recovery/history contracts; injected live outage and process-crash exercises pending. |
...
| HIS-003 | Additive `dictation_metrics_v1` migration creates one transcript-free, non-cascading row per finalized nonsecure attempt and safely backfills existing history without fabricated timings. | Fresh/existing migration, schema denylist, idempotence, revision-upsert, legacy-null tests, installed backup/migration, and real attempt. | Automated-verified and installed-runtime verified: the pre-migration backup passed `quick_check`; all 21 existing history rows remained; 21 legacy rows backfilled with unavailable timing/engine fields `NULL`; five real attempts produced five distinct measured UUIDs with zero duplicates, including the final short gate attempt. |
| HIS-004 | Local analytics, destination metadata, transcript history, retained debug sessions, and full-data deletion have explicit controls; opt-out or history failure never blocks dictation. | Preference, destination-redaction, actor-deletion, persisted-file scrubbing, debug-directory deletion, retention, and concurrent read-only SQLite tests. | Automated-verified for storage/deletion contracts; Settings and live history-outage exercises pending. |

## Privacy and reliability

| ID | Acceptance requirement | Evidence/method | Current state |
|---|---|---|---|
| PRV-001 | No desktop audio, telemetry, automatic updater, or desktop cloud-ASR traffic. | Dependency/source audit plus runtime network capture. | Static desktop privacy audit passes; runtime network capture pending. |
| IPH-001 | Disabled endpoint opens no listener; enabled endpoint binds only loopback and adds less than 5% desktop latency/less than the existing 0.5% idle CPU gate. | Socket inspection, Instruments, and desktop signpost comparison. | Source/build observed; runtime measurement pending. |
| IPH-002 | Mac-ready requests prefer local ASR; app/tunnel/model outages and preemption visibly use cloud fallback only when consented. | Worker/unit contracts plus staged real-device matrix. | Automated Worker routing and lease contracts present; deployed and iPhone-M4A verification pending. |
| IPH-003 | Remote request files are deleted, transcripts stay out of Worker storage and logs and out of Mac history unless the opt-in Unified iPhone History setting is on, and every HTTP response is no-store. | File/database/log inspection and response contract tests. | Source and automated contracts partially verified; deployed inspection pending. |
| IPH-004 | Gateway and origin are separately Access-protected and use different revocable Service Auth tokens. | Cloudflare configuration readback plus anonymous/authenticated probes. | Pending deployment and readback. |
| IPH-005 | Three distinct service tokens isolate the Shortcut, the iPhone web app, and the Worker-to-origin path. The gateway application is path-scoped to `/v1`; a PWA Shell Bypass application explicitly covers `/app` and `/stream`; `/app/` loads anonymously with a Content-Security-Policy header; and every token cross-use is rejected. Public `/stream` requires a same-origin 30-second signed ticket minted through protected `/v1`; no Access token enters the socket URL. | The ten anonymous/authenticated/ticket-refusal probes in `scripts/configure-iphone-endpoint.sh`, ticket tamper/expiry tests, plus Cloudflare application and policy readback. | Probe and ticket logic are automated; pending deployment, readback, and a per-token revocation exercise. |
| IPH-006 | Unified history synchronization is correct: repeated `import` operations and replayed `op_id` values are idempotent, `edit`/`pin`/`unpin`/`delete` enforce `base_revision` and return `conflict` with current server state otherwise, the global revision advances once per applied batch, and a `history_changed` response during paging restarts the snapshot without corrupting the cache. | Mac-side operation and revision contract tests, gateway proxy tests, and a staged two-device exercise. | Pending: implementation lands in other lanes and no runtime evidence is attached. |
| IPH-007 | The iPhone web app persists no audio, never caches `/v1/*`, clears the import fragment with `history.replaceState` before rendering, and stores the PWA token only in IndexedDB where it is redacted in the UI and removable by Remove key and confirmed Remove all data from this phone. Its separate diagnostics store retains at most 100 allowlisted, transcript-free events and supports Copy/Clear actions. | Static PWA asset audit, diagnostic allowlist/bounds tests, service-worker and cache-header inspection, IndexedDB inspection, and Safari address-bar/history observation on a real device. | Static asset and diagnostic contracts are automated; runtime storage, cache, fragment, and real-device copy inspection are pending. |
| IPH-008 | The real-device matrix passes: install to Home Screen and relaunch standalone, record a real AAC clip, the entry appears on both the phone and the Mac, a cloud-fallback result imports through the Shortcut fragment link, offline mutations queue and drain, revoking one token locks out only its client, Worker logs contain no content-bearing fields, and 90-day pruning removes unpinned entries while sparing pinned ones. | Staged real-device run on the supported iPhone and Mac, Cloudflare Worker log inspection, and a clock-advanced or seeded retention exercise. | Pending manual; no device run has been performed. |
| IPH-009 | Dictation Inbox streams PCM and displays partial text when supported, the same frames produce a valid temporary WAV for authoritative batch ASR, and ticket/setup/socket/preemption/finalization failures automatically use the retained file with the same request UUID. Neither transport persists audio or leaks content through diagnostics. | Worklet/transport fallback tests, HMAC proxy tests, live localhost WebSocket test, streamed-WAV/session lifecycle tests, privacy audit, and real iPhone comparison. | Source and automated protocol/session contracts pass; deployment, Safari AudioWorklet behavior, latency measurement, and real-device fallback remain pending. |
...
| PRV-004 | Temporary WAV deletion and crash-orphan cleanup are reliable; debug retention is explicit, capped at 10, and wholly deletable. | File-system integration/crash tests. | Retained debug transcript metadata and orphan-file deletion are automated-verified; live capture plus launch/cancellation/crash exercise pending. |
...
| PRV-006 | Local analytics contain no transcript/audio/URL/title/content/error columns; secure fields emit no history or metric row; no metric is transmitted. | Schema denylist, source-flow audit, opt-out tests, installed SQLite inspection, and runtime network capture. | Schema/source contracts and installed SQLite inspection are verified. Home Base read 26 valid events through its canonical read-only allowlist with zero invalid timestamps and reported no transcript/audio field access; secure-field runtime and independent network capture remain pending. |
...
The latest source hardening and focused regression evidence are recorded in
[hardening-validation.md](hardening-validation.md). Its synthetic/browser results do not replace the
real-device gates above.
...
| TOOL-002 | Direct SwiftPM invocation through the Xcode toolchain works when supplied the SDK plus Developer Framework search paths, even though `xcrun` remains broken. | Current run built the app and passed 191 tests in 30 suites. | Automated source contracts are usable on this host; the documented setup script contains the workaround. Repairing Xcode remains advisable for normal tooling. |
...
| ID | Contract | Automated evidence | Remaining acceptance |
|---|---|---|---|
| RW-001 | Unknown/protected selection never reads clipboard automatically; explicit recovery follows voice/options intent. Own-process invocation uses labeled clipboard routing. | Injected source/recorder/clipboard tests; lazy range/text classification tests. | Installed-app protected-field and custom-editor selection behavior. |
| RW-002 | Close/resume preserves completed manual edits; canceled generation cannot overwrite them; edits to Original survive navigation within the five-draft bound. | Model/controller regression tests including late provider output and provenance. | Installed-app voice revision and window-close paths. |
| RW-003 | Insert After does not move the caret in preflight; final validation and clipboard ownership precede paste. | Plain/rich TextEdit acceptance includes simulated post-preflight failure, Replace/Insert/Undo and concurrent-copy unit tests. | Other editors remain Copy-only; Spaces/full-screen remain manual. |
| RW-004 | Instructions Return submits, Shift+Return adds a newline; handled Return/Escape also consume their keyups. | Real TextEdit popup keyboard acceptance plus synthetic event-routing tests. | Installed global hotkey conflicts. |
| RW-005 | Dismissing a dictation rewrite preserves its original in ordinary Preview before or after finalization. | Coordinator effect tests and source-verified AppDelegate recovery/history-await ordering. | Real microphone handoff and Escape timing. |
| RW-006 | iPhone requests do not cancel local writing, including canceled inference still draining; desktop retains priority. | Writer drainage and inference admission tests. | Real iPhone request during local rewriting; visible fallback behavior. |

Current commands, results, and manual steps: [rewrite reliability validation](rewrite-reliability-validation.md).
