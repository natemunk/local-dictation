...
| UX-006 | Escape always cancels active/in-flight work and discards temporary audio. | Unit plus temporary-file integration test. | Generation detachment and immediate restart are automated-verified; finish capture is session-owned and rechecks generation after AX suspension. App/file lifecycle still needs manual integration. |
...
| IPH-007 | The iPhone web app persists no audio, never caches `/v1/*`, clears the import fragment with `history.replaceState` before rendering, and stores the PWA token only in IndexedDB where it is redacted in the UI and removable by Clear Credentials and Delete All Local Cache. Its separate diagnostics store retains at most 100 allowlisted, transcript-free events and supports Copy/Clear actions. | Static PWA asset audit, diagnostic allowlist/bounds tests, service-worker and cache-header inspection, IndexedDB inspection, and Safari address-bar/history observation on a real device. | Static asset and diagnostic contracts are automated; runtime storage, cache, fragment, and real-device copy inspection are pending. |
