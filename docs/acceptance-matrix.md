...
| TOOL-004 | Source packaging puts SwiftPM resource bundles under `Contents/Resources`, ad-hoc signs the result, and restores its generated dependency checkout. | Isolated-home `./setup --no-launch --skip-tests`, strict code-sign verification, bundle metadata/architecture checks, and clean dependency checkout. | Automated/source-build packaging verified on this host; notarization and downloadable binary distribution remain deferred. |
