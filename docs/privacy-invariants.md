...
| PRV-013 | Raw and delivered transcript history is actor-isolated local SQLite/WAL data with 90-day retention for successful, failed, pending, and cancelled rows by default. | Purge expired rows at launch and daily; delete-entry and delete-all are complete and local. | GRDB migrations, WAL health, concurrent actor access, all-state retention, FTS5/escaped search, and deletion tests pass. |
